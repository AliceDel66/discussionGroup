...
**所有产品/功能文档必须归档在 `projects/<project-slug>/` 下**，不得在仓库根目录直接创建 `product/`、`plans/` 等业务目录。

新建项目时 **必须**：
1. 创建 `projects/<project-slug>/` 及子目录
2. 编写 `projects/<project-slug>/README.md` 作为**项目入口索引**
3. 更新 `projects/README.md` 与根目录 `README.md` 的项目列表

### 仓库一级目录

```
discussionGroup/
├── agent.md                 # 本文件：Agent 全局操作指南
├── README.md                # 仓库总览 + 项目索引
└── projects/                # 所有项目归档（核心）
    ├── README.md            # 项目目录说明 + 项目列表
    └── <project-slug>/      # 单个项目大文件夹（见下）
```
...
├── README.md                # 【必填】项目入口：状态、文档索引、待确认项、Codex 上手
├── ideas/                   # 讨论记录（L0+）
├── product/
│   └── spec.md              # 产品规格（L2+）；复杂项目可用 specs/<feature>.md
├── plans/
│   └── plan.md              # Codex 任务规划（L3）
├── tech/
│   └── architecture.md      # 技术架构（可选，L2+ 建议有）
├── business/                # 商业模式、市场（可选）
├── research/                # 调研、竞品（可选）
├── decisions/               # ADR 决策记录
├── questions/               # 开放问题池
...
- 英文小写 + 连字符，如 `pharma-rep-agent`、`order-system`
- 与产品代号一致，稳定后不改名；改名需更新所有交叉链接

### 文件命名（项目内）

| 类型 | 路径 |
|------|------|
| 项目索引 | `projects/<project-slug>/README.md` |
| 讨论记录 | `ideas/YYYY-MM-DD-<主题>.md` |
| 产品规格 | `product/spec.md` 或 `product/specs/<feature>.md` |
| Codex 规划 | `plans/plan.md` 或 `plans/<feature>-plan.md` |
| 技术架构 | `tech/architecture.md` |
| 商业分析 | `business/business.md` 或 `business/<主题>.md` |
| ADR | `decisions/YYYY-MM-DD-<主题>.md` |
...
| 项目简介 | 一句话 + 目标用户 |
| 项目状态 | 等级、状态、版本 |
| 文档索引 | 链接到 spec / plan / architecture / business / ideas |
| 目录结构 | 本项目文件夹树 |
| 给 Codex 的快速上手 | 3–5 步 |
| 待确认项 | 阻塞开发的未决问题 |
| 讨论记录索引 | 按时间倒序 |
...
以下根目录扁平结构**已废弃**，旧文档应迁入 `projects/<slug>/`：
...
|------|------|----------|----------------|
| L0 灵感 | `探索中` | `projects/<slug>/ideas/` 下简短记录 | 否 |
| L1 方向 | `待验证` | 问题陈述、目标用户、成功指标 | 否 |
| L2 产品定义 | `有结论`（产品层） | `product/spec.md`：逻辑图、主流程、边界 | 待 plan 后 |
| L3 可开发包 | `可开发` | spec + `plans/plan.md`：任务拆解、验收标准 | 是 |
| L4 决策/变更 | `有结论` | `decisions/` ADR + 更新 spec/plan | 视变更而定 |
...
| 数据实体关系 | `erDiagram` |
| 系统模块与边界 | `graph` 架构图（放 `projects/<slug>/tech/architecture.md`） |
...
1. **判断是否已有项目**：若讨论属于已有产品，进入 `projects/<project-slug>/`；若是全新方向，先创建项目文件夹与 `README.md`。
2. 功能型讨论写入 `product/spec.md`；达到 L3 时同步创建或更新 `plans/plan.md`。
3. 讨论记录写入 `ideas/YYYY-MM-DD-<主题>.md`。

### 3. 写入文件

#### A. 讨论记录（每次必有）

路径：`projects/<project-slug>/ideas/YYYY-MM-DD-<主题>.md`

#### B. 功能规格（L2 及以上）

路径：`projects/<project-slug>/product/spec.md`（单 spec 项目）或 `product/specs/<feature>.md`（多 feature 项目）
...
- 技术架构：`../tech/architecture.md`
- 决策：`../decisions/...`
- 讨论：`../ideas/YYYY-MM-DD-....md`
```

#### C. Codex 任务规划（L3 / 状态为可开发）

路径：`projects/<project-slug>/plans/plan.md`
...
Codex 完成后应更新：本 plan 的任务勾选状态，并在 `product/spec.md` 中注明「已实现 / 偏差说明」。同步更新项目 `README.md` 状态。
```

#### D. 决策记录（ADR）

路径：`projects/<project-slug>/decisions/YYYY-MM-DD-<决策主题>.md`
...
路径：`projects/<project-slug>/README.md`
...
| 文档 | 路径 | 说明 |
|------|------|------|
| 产品 Spec | [product/spec.md](./product/spec.md) | … |
...
- 新建项目：更新 `projects/README.md` 项目列表 + 根目录 `README.md` 项目索引
- 项目内新增讨论：更新该项目 `README.md` 讨论记录索引
- 项目状态变更：更新该项目 `README.md` 顶部状态表
...
5. **验收化** — 每条功能有可测试的完成定义。
6. **任务化** — L3 时拆 Phase 与任务 ID，写入 `projects/<slug>/plans/plan.md`。
7. **归档** — 写入项目目录并更新项目 README + 根索引。
...
| 类型 | 适用场景 | 路径（均在 `projects/<slug>/` 下） |
|------|----------|-------------------------------------|
| `idea` | 灵感、讨论 | `ideas/` |
| `product` | 功能、流程、交互 | `product/spec.md` |
| `tech` | 选型、架构 | `tech/architecture.md` |
| `business` | 变现、增长 | `business/` |
| `research` | 竞品、调研 | `research/` |
| `decision` | 方案抉择 | `decisions/` |
| `question` | 仅有问题 | `questions/` |
| `plan` | Codex 任务包 | `plans/plan.md` |
| `index` | 项目入口 | `README.md` |
...
- 提交信息建议：
  - `docs: 归档 <项目>/<主题> 讨论 (YYYY-MM-DD)`
  - `docs(<project-slug>): 产品规格 / 任务规划 / 项目 README`
...
- [ ] 是否已归入 `projects/<slug>/` 并更新项目 README？
- [ ] 新建项目时是否已创建项目 README + 更新 `projects/README.md`？
- [ ] 是否写入正确路径并更新索引？
